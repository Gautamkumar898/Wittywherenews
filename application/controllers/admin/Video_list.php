...
#-----------------------------------------
    public function index($search=NULL) {
        ...
        // pagination settings
        $total_rows = $this->db->select('*')->from('video_url')->like("video_name",$keyword)->get()->num_rows();
        $limit = 15;
        $config["base_url"] = base_url("admin/Video_list/index");
        ...
        ->from("video_url")
        ->like("video_name",$keyword)
        ->limit($limit,$start)
        ->order_by('url_id','DESC')
        ->get()
        ->result_array();

        $this->load->view('admin/header', $header);
        $this->load->view('admin/menu');
        $this->load->view('admin/view_video_list');
